    ...
    */ function (search, record, file, https, runtime, format) {
    ...
    function postCashSale(data) {
        try {
            ...
            // Set header-level taxitem LAST, ignoreFieldChange true to prevent re-sourcing from customer
            var headerTaxCode = (items && items[0] && items[0].taxCode) ? items[0].taxCode : null;
            if (headerTaxCode) {
                dataRec.setValue({
                    fieldId: "taxitem",
                    value: headerTaxCode,
                ...
                log.debug('Set header taxitem', headerTaxCode);
            }
            for (var i = 0; i < items.length; i++) {
                ...
                // CRITICAL: Convert string to number for NetSuite
                let rate = parseFloat(data.items[i].rate);
                let amount = parseFloat(data.items[i].amount);
                ...
                // Set taxCode FIRST so NetSuite can calculate correctly
                if (taxCode) {
                    dataRec.setCurrentSublistValue({
                        sublistId: "item",
                        fieldId: "taxcode",
                    ...
                    log.debug('before commit with taxCode')
                } else {
                    dataRec.setCurrentSublistValue({
                        sublistId: "item",
                        fieldId: "rate",
                    ...
                    // DO NOT set amount - let NetSuite calculate it automatically
                    log.debug('Rate set, amount will be auto-calculated', {
                        rate: rate,
                        quantity: quantity,
                        expectedAmount: rate * quantity
                    });
                    log.debug('before commit with non taxCode')
                }
                dataRec.commitLine({
                    sublistId: "item",
                });
            ...
            // After save, force-update taxitem via submitFields to bypass all field change events
            if (headerTaxCode) {
                try {
                    record.submitFields({
                        type: record.Type.CASH_SALE,
                        id: cashSaleID,
                        values: { taxitem: parseInt(headerTaxCode, 10) },
                        options: { enableSourcing: false, ignoreMandatoryFields: true }
                    });
                    log.debug('submitFields taxitem success', headerTaxCode);
                } catch (e) {
                    log.debug('submitFields taxitem failed', e);
                }
            }
