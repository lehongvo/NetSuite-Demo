...
// Reuse the input shape from data.json (same as createOrderToNetsui.go)
type Input struct {
	Payload struct {
...
// Minimal Restlet payload (matches abj_cpos_integration.js expectations)
...
func main() {
	...
	// Initialize cache/redis (aligning with NewCashSaleService dependency shape)
	cacheSvc := initCacheService()
	_ = cacheSvc // kept for parity with requested wiring; no service instantiation to avoid extra deps
...
func buildCashSaleRequest(order Order, rawPayload json.RawMessage) cashSaleRequest {
	transDate := envOrDefault("NS_TRAN_DATE", timeNow()) // dd/mm/yyyy
...
func initCacheService() *cache.Service {
	redisRead := envOrDefault("REDIS_ADDR_READ", os.Getenv("REDIS_ADDR"))
	if redisRead == "" {
		redisRead = "127.0.0.1:6379"
	}
	redisWrite := envOrDefault("REDIS_ADDR_WRITE", os.Getenv("REDIS_ADDR"))
	if redisWrite == "" {
		redisWrite = redisRead
	}
	redisPassword := os.Getenv("REDIS_PASSWORD")

	redisClientRead := redis.NewClient(&redis.Options{
		Addr:     redisRead,
		Password: redisPassword,
		DB:       8,
	})
	redisClientWrite := redis.NewClient(&redis.Options{
		Addr:     redisWrite,
		Password: redisPassword,
		DB:       8,
	})

	return cache.NewService(redisClientRead, redisClientWrite)
}
...
func timeNow() string {
	now := time.Now()
	return fmt.Sprintf("%02d/%02d/%04d", now.Day(), int(now.Month()), now.Year())
}
