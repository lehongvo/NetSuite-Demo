    ...
    function postCashSale(data) {
        try {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                let quantity = parseFloat(data.items[i].quantity);
                ...
                if (isUseBin) {
                    ...
                    /**
                     * Bin allocation logic (with safety):
                     * - try: collect bins with qty > 0 at this location, sort by bin internal ID ASC (oldest first), then allocate quantity across bins.
                     * - catch: simple fallback using raw search result without sorting.
                     */
                    try {
                        log.debug('Go try bin allocation (sorted by internal ID ASC)');
                        let allBins = [];
                        ...
                        itemSearchObj.run().each(function (result) {
                            ...
                            let qtyAvailable = parseInt(result.getValue({ name: "quantityavailable", join: "binOnHand" }), 10);
                            log.debug('hasil saved search (fallback)', {idBin : idBin, qtyAvailable : qtyAvailable})
                ...
                let rate = parseFloat(data.items[i].rate);
                let amount = parseFloat(data.items[i].grossAmt);
                let taxCode = data.items[i].taxCode;
                
                log.debug('Item values', {
                    itemID: itemID,
                    rate: rate,
                ...
                if (!rate || isNaN(rate)) {
                    log.error('Invalid rate', 'Rate is required and must be a number for item: ' + itemID);
                }
                if (!amount || isNaN(amount)) {
                    log.error('Invalid amount', 'Amount is required and must be a number for item: ' + itemID);
                }
                if (internalID) {
                    dataRec.selectLine({
                        sublistId: "item",
                ...
                // Set taxCode and price based on whether tax is applicable
                if (taxCode) {
                    dataRec.setCurrentSublistValue({
                        sublistId: "item",
                        fieldId: "taxcode",
                    ...
                    log.debug('taxCode set', taxCode);
                } else {
                    log.debug('taxCode not provided, using item default');
                }
                dataRec.setCurrentSublistValue({
                    sublistId: "item",
                    fieldId: "price",
                ...
                // SAFE APPROACH: Handle both scenarios - with and without tax calculation
                if (taxCode && amount) {
                    log.debug('Using reverse-calculation method with taxCode', {
                        taxCode: taxCode, 
                        amount: amount,
                        quantity: quantity
                    });
                    ...
                    log.debug('NetSuite calculated amount (excluding tax)', amtLinet);
                    
                    var rateToset = Number(amtLinet) / Number(quantity);
                    dataRec.setCurrentSublistValue({
                        sublistId: "item",
                        fieldId: "rate",
                        value: rateToset,
                    ...
                    log.debug('Calculated rate from amount', {
                        rateToset: rateToset, 
                        amountExcludingTax: amtLinet, 
                        quantity: quantity
                    });
                } else {
                    log.debug('Using direct rate method', {
                        rate: rate,
                        quantity: quantity,
                    ...
                    log.debug('Rate set, amount will be auto-calculated', {
                        rate: rate,
                        quantity: quantity,
                        expectedAmount: rate * quantity
                    });
                }
                dataRec.commitLine({
                    sublistId: "item",
